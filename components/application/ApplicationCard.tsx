...
export function ApplicationCard({ 
  ...
  return (
    <Card className={`relative overflow-hidden border-0 shadow-lg bg-white dark:bg-zinc-950 ring-1 transition-all duration-300 ${hasEscrow ? 'ring-emerald-500/40 shadow-emerald-100/20' : 'ring-zinc-200 dark:ring-zinc-800'}`}>
      <CardContent className="p-0">
        <div className="flex flex-col lg:flex-row">
            ...
            <div className="space-y-3">
              ...
              {hasEscrow ? (
                <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
                  ...
                  <div className="mt-2">
                    <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-200 text-[9px] font-black animate-pulse">
          ...
          {/* 3. Dynamic Navigation & Contextual Actions */}
          <div className="p-6 flex flex-col justify-center gap-2.5 min-w-[240px] bg-white dark:bg-zinc-950">
            {app.status === "PENDING" ? (
              ...
              <div className="space-y-2 w-full">
                {/* DEDICATED ROUTING BUTTONS */}
                <Link href={`/landowner-dashboard/navigate/malpot/${landId}`} className="w-full">
                  <Button variant="outline" size="sm" className="w-full justify-start rounded-xl h-9 text-[11px] font-bold border-amber-200 text-amber-700 bg-amber-50/50 hover:bg-amber-100 transition-colors">
                ...
                <Link href={`/landowner-dashboard/navigate/land/${landId}`} className="w-full">
                  <Button variant="outline" size="sm" className="w-full justify-start rounded-xl h-9 text-[11px] font-bold border-blue-200 text-blue-700 bg-blue-50/50 hover:bg-blue-100 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  ...
                  >
                    <MessageSquare className="h-3.5 w-3.5 mr-1.5 text-emerald-600" /> Chat
                  </Button>
                  
                  <Link href={`/verify-agreement/${escrow.id}`}>
                    <Button variant="ghost" size="sm" className="w-full rounded-xl h-9 text-[10px] font-bold">
                      <Paperclip className="h-3.5 w-3.5 mr-1.5" /> Verify with Malpot papers
                    </Button>
                  </Link>
                </div>
              </div>
