...
import { useGetEscrowById, useSubmitMalpotPapers } from "@/queryandmutation/index";
import { useUploadThing } from "@/lib/useUploadthings"; // The hook you exported
import { FileUploadDemo } from "@/components/landowner/uploadfile"; // Your custom component
...
export default function VerifyAgreementPage() {
  ...
  console.log("Escrow Data:", escrow); // Debugging line to check fetched data
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Upload the signed Malpot papers for <span className="text-foreground font-medium">{escrow?.application.land.title}</span>.
        </p>
      </div>
          ...
          <Card className="p-4 border-dashed bg-muted/30">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className={escrow?.landownerMalpotUrl ? "text-emerald-600 font-medium" : "text-amber-600 font-medium"}>
                  {escrow?.landownerMalpotUrl ? "Uploaded" : "Pending"}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Landleaser:</span>
                <span className={escrow?.landleaserMalpotUrl ? "text-emerald-600 font-medium" : "text-amber-600 font-medium"}>
